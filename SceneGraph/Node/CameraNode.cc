                       ...
                       GLfloat far) : SceneNode("Camera", SceneNodeProperties::RenderPass::RenderPass_0, glm::mat4(1.0f))
{
  ...
  // Right vector
  glm::vec3 right = glm::vec3{sin(m_horizontalAngle - 3.14f/2.0f),
                              0,
...
void CameraNode::VOnUpdate(Scene * pScene, const double elapsedTime)
{
  ...
  //	TODO : Fix that hardcoded dimensions.
  glfwSetMousePos(WIDTH/2, HEIGHT/2);

  // Compute new orientation
  m_horizontalAngle += m_mouseSpeed * float(WIDTH/2 - xpos );
  m_verticalAngle   += m_mouseSpeed * float(HEIGHT/2 - ypos );

  // Direction : S.hherical coordinates to Cartesian coordinates conversion
  glm::vec3 direction(
    cos(m_verticalAngle) * sin(m_horizontalAngle),
    sin(m_verticalAngle),
    cos(m_verticalAngle) * cos(m_horizontalAngle)
  ...
  if (glfwGetKey( GLFW_KEY_LSHIFT ) == GLFW_RELEASE)
  {
    // Move forward
  ...
  else
  {
    // Move up
    if (glfwGetKey( GLFW_KEY_UP ) == GLFW_PRESS){
      up += direction * fElapsedTime * m_speed;
    }
    // Move down
    if (glfwGetKey( GLFW_KEY_DOWN ) == GLFW_PRESS){
      up -= direction * fElapsedTime * m_speed;
  ...
  // Camera matrix
  m_View = glm::lookAt(
    m_position,           // Camera is.hhere
    m_position+direction, // and looks.hhere : at .hhe same position, plus "direction"
