...
int GLManager::InitOpenGL(size_t _width, size_t _height, const char * title)
{
  ...
  if(!GLEW_ARB_vertex_array_object)
    ...
    std::cout << "ARB_vertex_array_object available." << std::endl;
...
bool GLManager::InitBlur()
{
  ...
  if ((status = glCheckFramebufferStatus(GL_FRAMEBUFFER)) != GL_FRAMEBUFFER_COMPLETE) {
    fprintf(stderr, "glCheckFramebufferStatus: error %i", status);
    return false;
  }
...
void GLManager::Loop()
{
  ...
  double deltaTime = 1000.0f * float(glfwGetTime() - lastTime);
